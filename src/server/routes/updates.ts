...
updates.get('/', async (c) => {
  logger.info('GET /api/v1/updates');
...
// GET /api/v1/updates/check - Trigger manual check for all games
updates.get('/check', async (c) => {
  logger.info('GET /api/v1/updates/check');
...
updates.post('/:id/grab', async (c) => {
  ...
  try {
    ...
    return c.json({ success: true, message: 'Update grab initiated' });
  } catch (error) {
...
updates.post('/:id/dismiss', async (c) => {
  ...
  try {
    ...
    return c.json({ success: true, message: 'Update dismissed' });
  } catch (error) {
...
// POST /api/v1/updates/games/:id/policy - Set update policy for a game
updates.post('/games/:id/policy', zValidator('json', updatePolicySchema), async (c) => {
  const id = parseInt(c.req.param('id'));
  logger.info(`POST /api/v1/updates/games/${id}/policy`);
