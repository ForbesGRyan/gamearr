...
export default function DownloadsTab({
  ...
  return (
    <>
      {/* Dry-Run Mode (top, prominent — affects every grab) */}
      <div
        className={`rounded-lg p-4 md:p-6 border-2 transition ${
          dryRun
            ? 'bg-amber-900/40 border-amber-600'
            : 'bg-gray-800 border-transparent'
        }`}
        role={dryRun ? 'alert' : undefined}
      >
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <h3 className="text-lg md:text-xl font-semibold mb-2 flex items-center gap-2">
              <svg
                className={`w-5 h-5 md:w-6 md:h-6 flex-shrink-0 ${
                  dryRun ? 'text-amber-300' : 'text-blue-400'
                }`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              ...
              Dry-Run Mode
              {dryRun && (
                <span className="text-xs uppercase tracking-wider bg-amber-600 text-amber-50 px-2 py-0.5 rounded">
                  Active
                </span>
              )}
            </h3>
          ...
          >
            {isSavingDryRun ? 'Saving...' : dryRun ? 'Disable' : 'Enable'}
          </button>
        </div>
      </div>
